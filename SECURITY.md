...
This project is pre-stable (**0.x**). Security fixes are applied on a best-effort basis to the latest `main` / development line only. There is no long-term support promise for older 0.x tags until a stable 1.x release ships with an explicit support window.
