...
**Terminal state:**
**Parent:**
**Closure proof:** composer.lock tracked; Dependabot; Illuminate ^11|^12 aligned. commit:aeb4a5c
...
- [x] `/composer.lock` is not ignored; composer.lock is tracked in git
- [x] `.github/dependabot.yml` covers composer (root), gomod (`/packages/capabilities-cli`), and github-actions
- [x] Illuminate constraints are identical across root + both PHP packages (either all include ^13 or none — document choice in commit message)
- [x] `composer validate` succeeds after constraint change
...
- packages/laravel-capabilities/composer.json
