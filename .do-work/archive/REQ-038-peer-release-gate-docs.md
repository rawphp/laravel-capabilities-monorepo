...
**Terminal state:**
**Parent:** REQ-035
**Closure proof:** checkpoint:.do-work/runs/RUN-036.yml#REQ-038 commit:4512889 tests:passed
...
- [x] Package README (or docs) has a “Peer support / D-011 release gate” section listing: matrix file location, required unit contract suite filters, fail/disable boot behaviour, AdapterApi bump rule
- [x] Section explicitly states default package CI does **not** install live laravel/ai or laravel/mcp
- [x] Section documents optional consumer peer-live steps (install peers, run app tests, confirm matrix cell)
- [x] Spec D-011 section is not contradicted (update only if needed for accuracy, do not delete design bible)
- [x] Unit test or lightweight check may guard README section anchors if chosen — still unit-only
...
- packages/laravel-capabilities/README.md — Peer support / D-011 release gate section
- docs/spec.md — D-011 accuracy for unit-only package CI
- packages/laravel-capabilities/tests/Unit/Adapters/PeerReleaseGateDocsTest.php — README anchor guards
