...
**Terminal state:**
**Parent:** REQ-039
**Closure proof:** checkpoint_log:passed commit:7126f1e
**Criteria approved:** agent-drafted
**Priority:** 1
**Size:** M
**Files:** docs/spec.md docs/tutorials/first-capability.md packages/laravel-capabilities/README.md packages/laravel-capabilities/tests/Unit/TestingHelpers/D020ConsumerDocsTest.php packages/laravel-capabilities/tests/Unit/TestingHelpers/HelperSurfaceTest.php docs/requirements-inventory.md tools/generate_requirement_stubs.py
...
- [x] Package README (or `docs/` testing section linked from it) documents `assertSchemaSnapshot` and `assertParity` with real argument shapes matching implementation
- [x] Spec D-020 prose matches shipped helper behaviour (update only if implementation differs from old example — keep dual-path prevention intent)
- [x] First-capability tutorial (if already added by REQ-042) links or embeds a minimal testing snippet using the real helpers
- [x] No documentation claims empty-arg assertParity alone proves multi-surface parity
- [x] Unit suite for TestingHelpers does not retain “exists for package consumers” as the only assert for parity/snapshot
- [x] Inventory status for D-020 scenarios is consistent with suite after sync (or documented intentional deltas)
...
- packages/laravel-capabilities/README.md — D-020 testing helpers section
- docs/spec.md — D-020 prose
- docs/tutorials/first-capability.md — §7 helpers
- packages/laravel-capabilities/tests/Unit/TestingHelpers/D020ConsumerDocsTest.php — docs guards
