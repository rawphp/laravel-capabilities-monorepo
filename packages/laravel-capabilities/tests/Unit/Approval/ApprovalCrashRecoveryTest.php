...
it("happy: resume re-validates and scoped re-resolves before run [P2-004]", function () {
    $called = false;
    $h = ApprovalHelpers::withPending(['revalidator' => function ($row) use (&$called) { $called = true; return null; }]);
    $id = (string) $h['row']['id'];
...
it("happy: resume emits approval.executed with via=resume [P2-004]", function () {
    ...
    foreach ($h['manager']->events() as $e) {
        if ($e instanceof CapabilityApprovalExecuted && $e->via === 'resume') { $found = true; }
    }
...
it("happy: accept emits approval.executed with via=accept [P2-004]", function () {
    ...
    $found = false;
    foreach ($h['manager']->events() as $e) {
        if ($e instanceof CapabilityApprovalExecuted && $e->via === 'accept') { $found = true; }
    }
    expect($found)->toBeTrue();
});
