...
def write_files(files: list[FileSpec]) -> dict:
    ...
    # Remove previously generated unit test trees carefully:
    # Only delete *.php under Unit and *_todo_test.go / generated go tests we own.
    ...
    for p in cli_root.rglob("*_todo_test.go"):
        p.unlink()
    # also remove generated non-todo names we may create
    for p in cli_root.rglob("*_test.go"):
        text = p.read_text(encoding="utf-8", errors="ignore")
        if "AUTO-GENERATED by tools/generate_requirement_stubs.py" in text:
            p.unlink()
...
def main() -> None:
    ...
    total = stats["cases"]
    print(f"Generated {total} cases across {stats['php']} PHP files and {stats['go']} Go files")
